...
# Load DEM raster
DEM_extended <- raster("lucie.zemanova/Ryby_Boris/gis_vrstvy/DEM_Aster_5514.tif")

# Ensure the coordinate reference systems match
# For the raster (terra object), use crs()
crs(DEM_extended) <- "EPSG:5514"

# Get coordinates of the river network segments (centroids)
river_net_simplified_centroids <- river_net_simplified %>%
  st_centroid()

# Ensure the centroids are in the same CRS as the DEM and shapefile
centroid_coords <- st_coordinates(river_net_simplified_centroids)
centroid_points <- st_as_sf(as.data.frame(centroid_coords), coords = c("X", "Y"), crs = st_crs(river_net_simplified))

# Extract elevation values from the DEM at centroid points
elevation_values <- terra::extract(DEM_extended, centroid_points)


# Add new columns for elevation and UPLAND_SKM values to the river shapefile
river_net_simplified <- river_net_simplified %>% 
  mutate(
    alt = elevation_values)      # Assign extracted elevation values

# Retain only the important columns
river_net_simplified <- river_net_simplified %>% 
  dplyr::select(NodeID, length, alt, UPLAND_SKM)

# View the updated data
print(river_net_simplified)

# Load necessary package
library(gridExtra)

# Plot 1: River network with elevation data
plot1 <- ggplot() +
  coord_fixed() +
...
# Plot 2: River network with UPLAND_SKM data
plot2 <- ggplot() +
  coord_fixed() +
  ggspatial::layer_spatial(river_net_simplified, aes(color = UPLAND_SKM)) +
  scale_color_viridis(name = "Accumulation area") +
  theme_minimal() +
  theme(legend.position = "bottom") +
  ggspatial::annotation_north_arrow(location = "br") +
  ggspatial::annotation_scale(location = "bl", style = "ticks")

# Use grid.arrange to display both plots on one page
grid.arrange(plot1, plot2, nrow = 1)


# river graph creating
min_elevation <- min(river_net_simplified$alt, na.rm = TRUE)
